...
class FurniverseSupabaseUser extends BaseAuthUser {
  FurniverseSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> furniverseSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FurniverseSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
